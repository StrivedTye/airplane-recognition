...
try:
    import model.pointnet2._ext as _ext
except ImportError:
...
class QueryAndGroup_score(nn.Module):
    ...
    def forward(self, xyz, new_xyz,score, features=None):
        ...
        # unique_score1 = torch.zeros((idx.shape[0], idx.shape[1]))
#         for i_batch in range(idx.shape[0]):
#             for i_region in range(idx.shape[1]):
#                 unique_ind = torch.unique(idx[i_batch, i_region, :]).long()
#                 unique_score1[i_batch, i_region] = score[i_batch,unique_ind].sum()
# #                score_cpu = score.detach().cpu().numpy()
# #                unique_ind_cpu = unique_ind.detach().cpu().numpy()
# #                aaa =1
        # unique_score_cpu = unique_score.detach().cpu().numpy()
        # unique_score1_cpu = unique_score1.detach().cpu().numpy()
        # print(unique_score_cpu)
        # print('............\n')
        # print(unique_score1_cpu)
