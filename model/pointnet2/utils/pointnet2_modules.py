...
from model.pointnet2.utils import pointnet2_utils
from model.pointnet2.utils import linalg_utils
import model.pointnet2.utils.pointnet2_utils
...
class PointnetSAModule(PointnetSAModuleMSG):
    ...
    def __init__(
        self, mlp, radius=None, nsample=None, bn=True, use_xyz=True
    ):
...
class PointnetSAModuleFeat(PointnetSAModuleMSG):
    r"""Pointnet set abstrction layer
    ...
    def __init__(
        ...
        # type: (PointnetSAModuleFeat, List[int], int, float, int, bool, bool) -> None
        super(PointnetSAModuleFeat, self).__init__(
            mlps=[mlp],
            radii=[radius],
            nsamples=[nsample],
            bn=bn,
    ...
    def forward(self, xyz, features, npoint):
        ...
        new_xyz = (
            pointnet2_utils.gather_operation(
                xyz_flipped, linalg_utils.FarthestPointSample(features, self.npoint)
                # xyz_flipped,torch.arange(self.npoint).repeat(xyz.size(0),1).int().cuda()
            )
                .transpose(1, 2)
                .contiguous()
