...
def attention(query, key, value, mask=None, dropout=None):
    ...
    # f = query.size(3)
    # n2 = key.size(2)
    # scores = F.cosine_similarity(query.unsqueeze(3).expand(B, h, n1, n2, f),
    #                              key.unsqueeze(2).expand(B, h, n1, n2, f),
    #                              dim=-1) #[B, h, n1, n2]
    # if mask is not None:
    #     scores = scores.masked_fill(mask == 0, -1e9)
    # p_attn = scores
...
class Transformer(nn.Module):
    def __init__(self, emb_dims=256, n_blocks=1, dropout=0, ff_dims=512, n_heads=4):
